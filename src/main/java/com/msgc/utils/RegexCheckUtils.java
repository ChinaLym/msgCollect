...
public class RegexCheckUtils {
	...
	public static final Pattern TABLE_KEY_PATTERN = Pattern.compile("[A-Za-z0-9]{4}");

	//校验表秘钥是否符合规范
	public static boolean checkTableKey(String tableKey){
		if(tableKey == null) {
			return false;
		}
		return TABLE_KEY_PATTERN.matcher(tableKey).matches();
	}
